...
router.post("/signup", (req, res) => {
    ...
    UserModel.create({ email: req.body.user.email, passwordhash: bcrypt.hashSync(req.body.user.password, 10) }) // PASSWORD ENCRYPTION 
        .then(returnedUser => {
...
router.post("/login", (req, res) => {
    UserModel.findOne({ where: { email: req.body.user.email } }) // findOne() method is a Sequelize method that does exactly what it says: it tries to find something within the database that we tell it to look for. This is called Data Retrieval.
        .then(returnedUser => {
            if (returnedUser) {
